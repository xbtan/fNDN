...
public:
  ...
  onInterest(const Interest& interest)
  {
      ...
      std::cerr << "<< I: " << interest <<"New Field: "<< interest.getFC()<<"(FC)"<<interest.getEC()<<"(EC)"<< std::endl;

        size_t Offset=interest.getFC();
        size_t End=interest.getEC();
        while(Offset<End+1)
        {
            std::cout<<"Offset= "<<Offset<<" End="<<End<<std::endl;
            std::cout<<">>D: "<<*m_store[Offset]<<std::endl;
            m_face.put(*m_store[Offset]);
            Offset++;      
        }
