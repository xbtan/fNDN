...
Consumer::run()
{
  try
    {
          Interest interest(Name(m_dataName),0,m_nTotalSegments-1);
...
Consumer::onData(Data& data)
{
  ...
  if (name[-1].toSegment() == m_nTotalSegments-1)
    {
    ...
    {
      Interest interest(Name(m_dataName),0,0);
      interest.setInterestLifetime(time::milliseconds(4000));
      interest.setMustBeFresh(m_mustBeFresh);
      m_face.expressInterest(interest,
                             bind(&Consumer::onData, this, _2),
...
Consumer::onTimeout(const Interest& interest)
{
  ...
  std::cerr << "TIMEOUT: last interest sent for segment #" << m_nextSegment << std::endl;
}
