...
Interest::Interest(const Name& name, const size_t FC, const size_t EC)
    :m_name(name)
    ,m_FC(FC)
    ,m_EC(EC)
    ,m_interestLifetime(time::milliseconds::min())
...
Interest::wireDecode(const Block& wire)
{
  ...
  //FC
  val = m_wire.find(tlv::FC);
  m_FC = size_t(readNonNegativeInteger(*val));
  //EC
  val = m_wire.find(tlv::EC);
    m_EC = size_t(readNonNegativeInteger(*val));
