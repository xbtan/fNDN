...
public:
  ...
  run()
  {
    m_face.setInterestFilter("/example/mytest",
...
private:
  ...
  onInterest(const InterestFilter& filter, const Interest& interest)
  {
    ...
    ifstream fin("/home/ndn/Documents/mydir/50m.wmv");    
    while(Offset < End + 1)
      {

      /*  Name dataName(flowname);
        */
        //change by jinyang
        char* buf=new char[MAX_SEG_SIZE];
        fin.read(buf,MAX_SEG_SIZE);
        ssize_t got=fin.gcount();// calculate the bytes 
