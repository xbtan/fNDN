...
void
Forwarder::InsertAgain(Face& inFace, const Interest& interest)
{
   size_t FC = interest.getFC();
   size_t EC = interest.getEC();
    std::string flowName = interest.getName().toUri();

    Interest f_interest(Name(flowName).appendSegment(1), FC, EC);
    static boost::random::uniform_int_distribution<uint32_t> dist;
    f_interest.setNonce(dist(getGlobalRng()));
    f_interest.setInterestLifetime(time::milliseconds(1000));
    f_interest.setMustBeFresh(true);

    shared_ptr<Interest> ff_interest = make_shared<Interest>(f_interest);

    const_cast<Interest&>(*ff_interest).setIncomingFaceId(inFace.getId());
    ++m_counters.getNInInterests();

  shared_ptr<pit::Entry> pitEntry = m_pit.insert(*ff_interest).first;
...
Forwarder::onIncomingInterest(Face& inFace, const Interest& interest)
{
  ...
  NFD_LOG_DEBUG("onIncomingInterest face=" << inFace.getId() <<
                " interest=" << interest.getName() << " FC=" << interest.getFC() << " EC=" << interest.getEC());

  //shared_ptr<Interest> f_interest = const_pointer_cast<Interest>(interest.shared_from_this());
 /* shared_ptr<Interest> f_interest= make_shared<Interest>(interest);
  f_interest->setName(Name(flowname));
  static boost::random::uniform_int_distribution<uint32_t> dist;
    f_interest->setNonce(dist(getGlobalRng()));
    f_interest->setInterestLifetime(time::milliseconds(1000));
    f_interest->setMustBeFresh(true);
                              ...
                              bool wantNewNonce)
{
  ...
  // insert OutRecord
  pitEntry->insertOrUpdateOutRecord(outFace.shared_from_this(), s_interest);

  // send Interest
  outFace.sendInterest(s_interest);
  ++m_counters.getNOutInterests();
}
...
Forwarder::onIncomingData(Face& inFace, const Data& data)
{
  ...
  ++m_counters.getNInDatas();
   ...
   /*
   //change 1
   //getPrefix() is get first part ,not get first byte;
   std::string namestr=data.getName().getPrefix(2).toUri();
   std::string instr("/example/mytest");

   if(instr==namestr)
    {
        std::ofstream fout("/home/jin/Documents/mydir/what.wmv",std::ofstream::app);
        Block block=data.getContent();
        for(Buffer::const_iterator  it=block.value_begin();it!=block.value_end();it++)
            {
                 fout<<*it;
            }
         fout.close();
    ...
    //change 2
   std::string namestr=data.getName().getPrefix(1).toUri();
   std::string instr("/example");
   std::cout<<"---------data----------"<<std::endl;
   std::cout<<data<<std::endl;
   if(instr==namestr)
    {
        //std::cout<<"name_data: "<<data.getName().toUri()<<std::endl;
        std::ofstream fout("/home/ndn/Documents/mydir/my50m.wmv",std::ofstream::app);
        //std::ofstream fout("/home/jin/Documents/mydir/myfile.txt",std::ofstream::app);
        Block block=data.getContent();
        //std::cout<<data<<std::endl;
        for(Buffer::const_iterator  it=block.value_begin();it!=block.value_end();it++)
            {
